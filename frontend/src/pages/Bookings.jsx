...
export default function Bookings() {
  ...
  const fetchBookings = async () => {
    try {
      const url = filter ? `/dashboard/bookings?status=${filter}` : '/dashboard/bookings';
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="font-semibold text-gray-800">{b.customer_name || 'Unknown'}</h3>
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${statusColors[b.status]}`}>
                      {b.status}
                    </span>
                  </div>
                  <p className="text-sm text-gray-500">📅 {b.booking_date} — ⏰ {b.booking_time}</p>
                  {b.customer_phone && (
                    <p className="text-sm text-gray-500 mt-1">📞 {b.customer_phone}</p>
                  )}
